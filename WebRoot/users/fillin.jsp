<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%><%@ page import="java.sql.*,java.util.*,java.util.Date"%>   
     <%@page import="java.util.ArrayList"%><%@taglib prefix="s" uri="/struts-tags" %>
...
function star()
{
	if (document.getElementById("tecnameMg").value == "") {
		window.alert("请填写机构简介！");		
		document.getElementById("tecnameMg").focus();
		return false;
	}
	...
	else if (document.getElementById("comandMg1").value == ""||document.getElementById("comandMg2").value == ""||document.getElementById("comandMg3").value == "") {
	...
	obj = document.getElementsByName("areaKinds");
	area = document.getElementsByName("areaKind");
    check_val = [];
    for(k in obj){
        if(obj[k].checked){
        	check_val.push(obj[k].value);
        	area.value+=check_val[k];	
        }
    }
}
function show()                           //隐藏某区域
...
<body>
<form action="addDemand.action" method="post" name="collectform" onsubmit = "return star();">
<%
		String NO="";
 ...
 <div align="center" style="margin-bottom:10px;display:none;" >
 	<p>表单号：<input name="demand.no" value="<%out.print(NO);%>" type="text"/>
 	机构全称：<input name="demand.userName" value="${user.username }" type="text" />
 	织机构代码：<input name="demand.id" value="${user.id }" type="text" id="postalAdds" />
 	审核状态：<input name="demand.shstate" value="0" type="text" id="shstate" />
 	提交状态：<input name="demand.tjstate" value="0" type="text" id="tjstate" />
 	形式审核状态：<input name="demand.xsstate" value="0" type="text" id="xsstate" />
 	部门审核状态：<input name="demand.bsstate" value="0" type="text" id="bsstate" /></p>
 </div>
<table border="1" align="center" cellpadding="4" cellspacing="0">
  <tr>
    <td width="247">机构全称</td>
    <td width="495">${user.username }</td>
  ...
  <tr>
    ...
    <td colspan="3"><textarea name="demand.tecnameMg" id="tecnameMg" placeholder="（单位基本情况，研究基础等，限500字）" 
  ...
  <tr>
    ...
    <td><input name="demand.cmadName" type="text" id="cmadName" > </td>
	<td>需求年限 </td> 
